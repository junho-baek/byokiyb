...
import { writeProjectEnvValue } from "./envFile.js";
...
async function handleSubmit(req, res, requestId, overwrite) {
  ...
  const writeResult = await writeProjectEnvValue({ projectPath: session.projectPath, envFile: session.envFile, keyName: session.keyName, value, overwrite });
  session.status = "written";
  session.present = true;
  session.fingerprint = fingerprintValue(value);
